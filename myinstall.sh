...
set -eE
confhome=https://raw.githubusercontent.com/pascalhack/mydd/main
...
setos() {
    ...
    setos_ros() {
        ...
        GATEGIWAY=$(ip route list | grep default | cut -d' ' -f 3) && \
        # 将配置写入挂载的镜像
        ...
        /interface ethernet set [ find default-name=ether1 ] name=WLAN
        /ip dns set server=114.114.114.114,223.5.5.5,223.6.6.6
        ...
        if [ $os_across_disks_count -eq 1 ]; then
            dd if=chr.img bs=1024 of=/dev/$xda && \
            reboot
        else
            echo 'Error: OS spans across multiple disks'
        fi
    }
    setos_alpine() {
